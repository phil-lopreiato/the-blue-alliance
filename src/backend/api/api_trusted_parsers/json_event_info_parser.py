...
# or a dict with 'url' as a key, which we can parse
class _WebcastUrlDict(Webcast, total=False):
    url: str
...
class JSONEventInfoParser:
    ...
    def parse(info_json: AnyStr) -> EventInfoParsed:
        info_dict = safe_json.loads(info_json, EventInfoInput)
        ...
        if "remap_teams" in info_dict:
            for temp_team, remapped_team in info_dict["remap_teams"]:
        ...
        if "playoff_type" in info_dict:
            if info_dict["playoff_type"] not in PlayoffType:
                raise ParserInputException(
